...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueV_Hcho != cur->valueV_Hcho)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueV_Hcho Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueV_Temper != cur->valueV_Temper)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueV_Temper Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueV_Humid != cur->valueV_Humid)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueV_Humid Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueV_Pm25 != cur->valueV_Pm25)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueV_Pm25 Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueV_Hcho = gizY2XFloat(V_HCHO_RATIO,  V_HCHO_ADDITION, dataPoints->valueV_Hcho); 

    devStatusPtr->valueV_Temper = gizProtocolExchangeBytes(gizY2XFloat(V_TEMPER_RATIO,  V_TEMPER_ADDITION, dataPoints->valueV_Temper));     
    devStatusPtr->valueV_Humid = gizProtocolExchangeBytes(gizY2XFloat(V_HUMID_RATIO,  V_HUMID_ADDITION, dataPoints->valueV_Humid));     
    devStatusPtr->valueV_Pm25 = gizProtocolExchangeBytes(gizY2XFloat(V_PM25_RATIO,  V_PM25_ADDITION, dataPoints->valueV_Pm25));     
